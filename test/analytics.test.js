...
test('analytics never turns missing observations into zero and rejects reversed ranges',t=>{const{service}=fixture(t);assert.throws(()=>service.summary({startAt:'2026-05-01T00:00:00.000Z',endAt:'2026-04-01T00:00:00.000Z'}),/ابتدای بازه/);const r=service.summary({projectId:'project-with-no-observations'});assert.equal(r.slump.statistics.count,0);assert.equal(r.slump.statistics.mean,null);assert.equal(r.strength.statistics.count,0);});
