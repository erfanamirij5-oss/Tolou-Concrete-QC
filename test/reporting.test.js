...
test('project QC report is company scoped and standards neutral',t=>{const{service}=fixture(t);const report=service.projectQc({projectId:'p1'});assert.equal(report.schema,'tolou-qc-project-report');assert.equal(report.schemaVersion,4);assert.equal(report.company.name,'شرکت یک');assert.equal(report.project.name,'پروژه یک');assert.equal(report.project.customerName,'مشتری الف');assert.equal(report.counts.samplingSeries,1);assert.equal(report.counts.approvedStrengthResults,1);assert.equal(report.series.length,1);assert.equal(report.qcDataset.length,1);assert.equal(report.analytics.strength.statistics.mean,31);assert.equal(report.filters.projectId,'p1');assert.equal(report.standards.profile,null);assert.equal(report.standards.acceptanceEvaluated,false);});
