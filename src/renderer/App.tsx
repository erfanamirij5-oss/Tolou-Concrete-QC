...
export function App(){
  ...
  const renderWorkspace=()=>{switch(activeView){
    ...
    case'projects':return <><section className="workspace-intro"><p className="eyebrow">عملیات</p><h2>پروژه‌ها و بتن‌ریزی‌ها</h2><p>پروژه را ایجاد کنید، بتن‌ریزی را ثبت کنید و پرونده کامل QC را در یک مسیر مدیریت کنید.</p></section><ProjectWorkbench onChanged={dataChanged} refreshKey={dataVersion} onStartSampling={goSampling} onOpenResults={()=>setActiveView('results')} onOpenReports={()=>setActiveView('reports')}/></>;
