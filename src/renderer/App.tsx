...
export function App() {
  ...
  useEffect(() => {
    ...
    Promise.all([window.tolou.health(), refreshDashboard()]).then(() => setHealth('ok')).catch(() => setHealth('error'));
  ...
  return <main className="app-shell">
    <aside className="sidebar glass glass--dark" aria-label="ناوبری اصلی">
      ...
      <nav className="nav-stack"><button className="nav-item nav-item--active"><span>◆</span>داشبورد</button><button className="nav-item"><span>◫</span>نمونه‌ها و نتایج</button><button className="nav-item"><span>▦</span>پروژه‌ها</button><button className="nav-item"><span>⌁</span>طرح‌های اختلاط</button><button className="nav-item"><span>◉</span>تحلیل و هشدارها</button><button className="nav-item"><span>▤</span>گزارش‌ها</button></nav>
    ...
    <section className="workspace">
      ...
      <div className="content-grid">
        ...
        <AnalyticsWorkbench refreshKey={dataVersion} />
