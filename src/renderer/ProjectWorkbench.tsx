...
export function ProjectWorkbench({onChanged,refreshKey=0,onStartSampling,onOpenResults,onOpenReports}:{onChanged?:()=>void;refreshKey?:number;onStartSampling?:(projectId:string)=>void;onOpenResults?:(projectId:string)=>void;onOpenReports?:(projectId:string)=>void}){
 ...
 async function reloadProjects(){const result=await window.tolou.listProjects();if(!result.ok)throw new Error(result.message);setProjects(result.data as ProjectWithStrength[]);}
 ...
 if(view==='new')return <section className="project-workbench glass"><div className="panel-heading"><div><p className="eyebrow">مرحله ۱</p><h3>ایجاد پروژه جدید</h3></div><button className="text-button" type="button" onClick={()=>setView('list')}>بازگشت</button></div>{message&&<div className="workbench-message" role="status">{message}</div>}<form className="workbench-form" onSubmit={submitProject}><label>نام پروژه<input name="name" required autoFocus/></label><label>نام مشتری<input name="customerName" required/></label><label>مقاومت مشخصه پروژه (MPa)<input name="characteristicStrengthMpa" type="number" min="0.1" step="0.1" required placeholder="مثلاً 30"/><small>مرجع ۷ روزه به‌صورت خودکار ۶۰٪ این مقدار و مرجع ۲۸ روزه برابر همین مقاومت خواهد بود.</small></label><label>آدرس پروژه<input name="address"/></label><button className="primary-button" disabled={busy}>{busy?'در حال ثبت…':'ایجاد پروژه'}</button></form></section>;
 ...
 if(view==='detail'&&current)return <section className="project-workbench glass project-dossier">
  ...
  {tab==='overview'&&<>
    <div className="strength-reference-strip">
      <div><small>مقاومت مشخصه پروژه</small><strong>{current.characteristic_strength_mpa==null?'—':`${current.characteristic_strength_mpa.toLocaleString('fa-IR',{maximumFractionDigits:2})} MPa`}</strong></div>
      <div><small>مرجع ۷ روزه · ۶۰٪</small><strong>{current.seven_day_reference_mpa==null?'—':`${current.seven_day_reference_mpa.toLocaleString('fa-IR',{maximumFractionDigits:2})} MPa`}</strong></div>
      <div><small>مرجع ۲۸ روزه · ۱۰۰٪</small><strong>{current.twenty_eight_day_reference_mpa==null?'—':`${current.twenty_eight_day_reference_mpa.toLocaleString('fa-IR',{maximumFractionDigits:2})} MPa`}</strong></div>
      <span>این مقادیر مرجع پروژه هستند؛ نتیجه آزمون همچنان مستقیماً به MPa ثبت می‌شود.</span>
    </div>
