...
export function createProjectService(db, {companyId}) {
  companyId = text(companyId, 'شرکت');
  return {
    createProject(input) {
      const id = text(input?.id, 'شناسه پروژه');
      const name = text(input?.name, 'نام پروژه');
      const customerName = text(input?.customerName, 'نام مشتری');
      ...
      return {id,name,customerName,address};
    },
    listProjects() {
      return db.prepare(`SELECT id,name,customer_name,address,archived FROM projects
        WHERE company_id=? ORDER BY archived,name,id`).all(companyId);
    },
    createPour(input) {
      const id = text(input?.id, 'شناسه بتن‌ریزی');
      const projectId = text(input?.projectId, 'پروژه');
      const occurredAt = utcTimestamp(input?.occurredAt);
      const project = db.prepare('SELECT id FROM projects WHERE id=? AND company_id=? AND archived=0').get(projectId, companyId);
      if (!project) throw new Error('پروژه فعال متعلق به این شرکت یافت نشد');
      if (db.prepare('SELECT id FROM pours WHERE id=?').get(id)) throw new Error('این شناسه بتن‌ریزی قبلاً ثبت شده است');
      db.prepare('INSERT INTO pours(id,company_id,project_id,occurred_at) VALUES(?,?,?,?)').run(id, companyId, projectId, occurredAt);
      return {id,projectId,occurredAt};
    },
    listPours(projectId) {
      projectId = text(projectId, 'پروژه');
      return db.prepare(`SELECT id,project_id,occurred_at FROM pours
        WHERE company_id=? AND project_id=? ORDER BY occurred_at DESC,id`).all(companyId, projectId);
    },
    dashboard() {
