...
export function createSpecimenPhysicsService(db,{companyId,actor,clock=()=>new Date().toISOString()}){
 ...
 return{
  ...
  listCurrent({limit=200}={}){const safe=Number.isSafeInteger(limit)&&limit>0&&limit<=500?limit:200;return db.prepare(`SELECT m.sample_id,m.revision,m.shape,m.length_mm,m.width_mm,m.height_mm,m.diameter_mm,m.mass_kg,m.volume_m3,m.density_kg_m3,m.reason,m.entered_by,m.entered_at FROM current_specimen_physical_measurements m JOIN samples s ON s.id=m.sample_id JOIN sampling_series ss ON ss.id=s.series_id WHERE ss.company_id=? ORDER BY m.entered_at DESC,m.sample_id DESC LIMIT ?`).all(companyId,safe);},
