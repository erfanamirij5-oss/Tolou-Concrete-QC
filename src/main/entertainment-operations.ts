...
async function resolveGamePath(owner:BrowserWindow|null){const bundled=bundledGamePath();if(existsSync(bundled))return bundled;const managed=managedGamePath();if(existsSync(managed))return managed;const choice=await dialog.showOpenDialog(owner??undefined,{title:'انتخاب فایل اصلی بازی طلوع',properties:['openFile'],filters:[{name:'Tolou HTML Game',extensions:['html','htm']}]});if(choice.canceled||!choice.filePaths[0])return null;mkdirSync(dirname(managed),{recursive:true});copyFileSync(choice.filePaths[0],managed);return managed;}
