...
import { createReportingService } from '../application/reporting.js';
import { persistLicense, readLicenseStatus, validateProductKey, type SubscriptionMonths } from './licensing.js';
...
async function collectFiles(root:string,current=root):Promise<BackupEntry[]>{if(!existsSync(current))return[];const result:BackupEntry[]=[];for(const entry of await readdir(current,{withFileTypes:true})){const absolute=join(current,entry.name);if(entry.isDirectory())result.push(...await collectFiles(root,absolute));else if(entry.isFile())result.push({path:absolute.slice(root.length+1).replace(/\\/g,'/'),data:(await readFile(absolute)).toString('base64')});}return result;}
async function createBackup(event:IpcMainInvokeEvent):Promise<SystemResult<{cancelled:boolean;path?:string}>>{try{const window=BrowserWindow.fromWebContents(event.sender);if(!window)throw new Error('پنجره برنامه در دسترس نیست');const selection=await dialog.showSaveDialog(window,{title:'ذخیره نسخه پشتیبان طلوع',defaultPath:`Tolou-QC-Backup-${new Date().toISOString().slice(0,10)}.tolouqcbackup`,filters:[{name:'Tolou QC Backup',extensions:['tolouqcbackup']}]});if(selection.canceled||!selection.filePath)return{ok:true,data:{cancelled:true}};const userData=app.getPath('userData');const snapshot=join(userData,`backup-snapshot-${Date.now()}.sqlite`);const db=openDb();try{db.exec(`VACUUM INTO ${sqlString(snapshot)}`);}finally{db.close();}const envelope:BackupEnvelope={magic:BACKUP_MAGIC,version:BACKUP_VERSION,createdAt:new Date().toISOString(),database:(await readFile(snapshot)).toString('base64'),attachments:await collectFiles(join(userData,'attachments'))};await rm(snapshot,{force:true});await writeFile(selection.filePath,gzipSync(Buffer.from(JSON.stringify(envelope),'utf8')));return{ok:true,data:{cancelled:false,path:selection.filePath}};}catch(error){console.error('[Tolou backup]',error);return{ok:false,message:'تهیه نسخه پشتیبان انجام نشد.'};}}
